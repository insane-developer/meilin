...
Storage.prototype = {
    ...
    view: function(){
        var thisObject = Array.prototype.pop.call(arguments),
            name = arguments[0];
        /* preparing arguments to be used in apply of call of template */
        arguments[0] = thisObject;
        
        return this.views[name] && Function.prototype.call.apply(this.views[name], arguments);
    ...
    makeTemplateFromString: function(str){
        ...
        return function(data){
            ...
            return str.replace(/\[%\s+([^\s%]+)\s+%\]/g, function(match, group){
                ...
                var nextTmpl = tmplNamespace[group];
                ...
                if(nextTmpl && nextTmpl.apply &&
                    ...
                    return tmplNamespace[group].apply(self, selfArgs);
                }
                return '';
            });
        }
